...
## CI menu regression follow-up

PR CI later exposed a timing race in the browser harness when reopening Menu: deferred mount autofocus could replace the test's focused Bravo item before Enter, leaving the menu open without a selection. Fifteen runs across normal, 4×, and 8× CPU throttling passed; a controlled delayed-frame and keyboard-dispatch schedule reproduced the failure and traced the focus change. The harness now waits for the reopening animation and a queued animation frame before selecting Bravo. The same controlled schedule passed five consecutive runs, and the complete menu browser family passed. The final `closed:bravo` assertion remains unchanged; this correction changes only test readiness.
